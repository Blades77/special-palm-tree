...
public interface TileRepository extends JpaRepository<TileEntity, Long> {
    List<TileEntity> getTileEntityByGroupDataEntity(GroupDataEntity groupDataEntity);
    ...
    List<TileEntity> getTileEntityByTagIdList(@Param("tagIdList") List<Long> tagIdList, @Param("listLength") int listLength, @Param("groupId") Long groupId);
    ...
    List<TileEntity> getTileEntityByAtLeastOneTagIdList(@Param("tagIdList") List<Long> tagIdList,@Param("groupId") Long groupId);

    @Query(value ="SELECT * FROM tile WHERE tile_title LIKE :string",nativeQuery = true)
    List<TileEntity> searchTileTitleByParam(@Param("string") String search);
}
